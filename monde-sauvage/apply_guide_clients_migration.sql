-- This script creates the guide_clients table if it doesn't exist
-- Run this in your Supabase SQL Editor or via CLI

-- First check if the table exists
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'guide_clients') THEN
        -- Create the table
        CREATE TABLE guide_clients (
        ...
        -- Indexes
        CREATE INDEX idx_guide_clients_guide_id ON guide_clients(guide_id);
        CREATE INDEX idx_guide_clients_name ON guide_clients(guide_id, full_name);
        CREATE INDEX idx_guide_clients_email ON guide_clients(guide_id, email) WHERE email IS NOT NULL;
        ...
        RAISE NOTICE 'guide_clients table created successfully';
    ELSE
        RAISE NOTICE 'guide_clients table already exists';
    END IF;
END $$;
