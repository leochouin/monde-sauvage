...
export default function SocialFeedModal({
  isOpen,
  onClose,
  user,
  guide,
  ...
  }, [isOpen, currentUserId, guide?.id, guide?.user_id]);
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      setOpenCommentsByPost(emptyOpenComments);
      setCommentsByPost(emptyComments);
      setLoadingCommentsByPost({});
      setCommentDraftByPost(emptyCommentDrafts);
      setCommentErrorByPost({});
      setSubmittingCommentByPost({});
      setFollowPendingByUser({});
  ...
  return (
    <div className="social-feed-overlay" onClick={onClose}>
      <div className="social-feed-modal" onClick={(event) => event.stopPropagation()}>
        <div className="social-feed-header">
          <div>
            <h2>Section Sociale</h2>
            <p>Partagez vos sorties, inspirez les pecheurs et suivez vos guides preferes.</p>
          </div>
          <button type="button" className="social-feed-close" onClick={onClose}>×</button>
        </div>
