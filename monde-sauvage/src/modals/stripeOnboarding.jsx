...
import {
  ...
  createEstablishmentDashboardLink,
...
const StripeOnboarding = ({ establishment, onStatusUpdate }) => {
  ...
  /**
   * Open the Stripe Express dashboard for this establishment via a
   * single-use login link generated by the stripe-dashboard-link function.
   */
  const handleOpenDashboard = async () => {
    setOpeningDashboard(true);
    setError(null);
    try {
      const { url } = await createEstablishmentDashboardLink(establishment.key);
      if (url) {
        window.open(url, '_blank', 'noopener,noreferrer');
      } else {
        throw new Error('Aucun lien de tableau de bord reçu');
      }
    } catch (err) {
