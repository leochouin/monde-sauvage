-- Expose establishment Stripe readiness on get_chalets_nearby so the client can
-- route chalet checkout to Stripe instead of the legacy insert-only flow.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(e.stripe_charges_enabled, FALSE)::BOOLEAN AS stripe_charges_enabled
  FROM chalets c
